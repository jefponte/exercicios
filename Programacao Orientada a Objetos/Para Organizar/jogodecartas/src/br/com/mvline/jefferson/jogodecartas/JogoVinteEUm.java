...
public class JogoVinteEUm extends Jogo {
	...
	/*
	 * 
	 * public void telaDeConfiguracao() { String nome = ""; int
	 * quantidadeDeBaralhos = 0; int numeroDeJogadoresHumanos = 0; int
	 * numeroDeJogadoresMaquina = 0; Scanner entrada = new Scanner(System.in);
	 * int comando = 0; do { comando = -1;
	 * System.out.println("Quantos Jogadores humanos?"); comando =
	 * entrada.nextInt(); numeroDeJogadoresHumanos = comando; } while (comando <
	 * 0);
	 * 
	 * do { comando = -1; System.out.println("Quantos Jogadores maquina?");
	 * comando = entrada.nextInt(); numeroDeJogadoresMaquina = comando;
	 * 
	 * } while (comando < 0); do {
	 * 
	 * comando = 0; System.out.println("Quantos Baralhos no jogo?"); comando =
	 * entrada.nextInt(); quantidadeDeBaralhos = comando; } while (comando < 1);
	 * for (int i = 0; i < numeroDeJogadoresHumanos; i++) {
	 * System.out.println("Digite o nome do jogar humano " + i + ": "); nome =
	 * entrada.next(); Jogador play = new Jogador(nome, true);
	 * this.jogadores.add(play); } for (int i = 0; i < numeroDeJogadoresMaquina;
	 * i++) { System.out.println("Digite o nome do jogar Máquina " + i + ": ");
	 * nome = entrada.next(); Jogador play = new Jogador(nome, false);
	 * this.jogadores.add(play); }
	 * 
	 * this.deck = new Deck(quantidadeDeBaralhos);
	 * 
	 * }
	 ...
	 * 
	 * for (Jogador jogador : this.jogadores) {
	 * this.deck.retirarUmaCarta(jogador); this.deck.retirarUmaCarta(jogador); }
	 * this.mostrarJogadores(); for (Jogador jogador : this.jogadores) {
	 * this.jogar(jogador); this.mostrarJogadores(); } }
	 * 
	 * public void jogar(Jogador jogador) { Scanner entrada = new
	 * Scanner(System.in); String resposta; int comando = 1;
	 * 
	 * do { System.out .println(jogador.getNome() +
	 * " é a sua vez de jogar. Digite \"comprar\" ou \"parar\":  ");
	 * if(jogador.getHumano()){ resposta = entrada.next(); }else { int
	 * pontosDaMaquina = this.retornaPontuacao(jogador);
	 * 
	 * if(pontosDaMaquina < 21){ resposta = "comprar"; }else{ resposta =
	 * "parar"; } }
	 * 
	 * switch (resposta) { case "comprar": this.deck.retirarUmaCarta(jogador);
	 * break; case "parar": comando = 0; break; case "sair":
	 * this.mostrarJogadores(); System.exit(0); break; case "exit":
	 * this.mostrarJogadores(); System.exit(0); break; case "help":
	 * System.out.println(JogoVinteEUm.REGRAS+ "\n"+ JogoVinteEUm.COMANDOS);
	 * break; default: System.out.println("Comando desconhecido."); break; }
	 * 
	 * } while (comando != 0);
	 * 
	 * }
